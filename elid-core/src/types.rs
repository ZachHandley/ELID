...
//!
//! This module defines the fundamental types used throughout the ELID system:
...
impl Elid {
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>, ElidError> {
        crate::decode_sortable(self.as_str())
    }
...
impl Profile {
    ...
    pub fn string_length(&self) -> usize {
        (self.bit_length() + 4) / 5 // 5 bits per base32hex char
    }
...
mod quantized_coords_tests {
    ...
    #[test]
    fn test_quantized_coords_edge_case_values() {
        ...
        let values = values.into_iter().cycle().take(128).collect::<Vec<_>>();
        let embedding = Embedding::new(values).unwrap();

        let coords = QuantizedCoords::from_embedding(&embedding, 5, 10).unwrap();

        let max_val = (1u16 << 10) - 1; // 1023

        // Verify first 5 values
        assert_eq!(coords.as_slice()[0], 0, "Expected 0 for -1.0");
        assert_eq!(coords.as_slice()[1], 512, "Expected 512 for 0.0");
        assert_eq!(coords.as_slice()[2], max_val, "Expected 1023 for 1.0");

        // 0.5 → (0.5 + 1) / 2 * 1023 = 0.75 * 1023 = 767.25 ≈ 767
        assert!(
            (coords.as_slice()[3] as i32 - 767).abs() <= 1,
            "Expected ~767 for 0.5, got {}",
            coords.as_slice()[3]
        );

        // -0.5 → (-0.5 + 1) / 2 * 1023 = 0.25 * 1023 = 255.75 ≈ 256
        assert!(
            (coords.as_slice()[4] as i32 - 256).abs() <= 1,
            "Expected ~256 for -0.5, got {}",
            coords.as_slice()[4]
        );
    }

    #[test]
    fn test_quantized_coords_different_bit_widths() {
        let values = vec![1.0; 128];
        let embedding = Embedding::new(values).unwrap();

        // Test various bit widths
        for bits in 1..=16 {
            let coords = QuantizedCoords::from_embedding(&embedding, 5, bits).unwrap();
            ...
            assert_eq!(coords.bits_per_dim(), bits);
            assert!(
                coords.as_slice().iter().all(|&c| c == expected_max),
                "For {} bits, expected max value {}, got {:?}",
                bits,
                expected_max,
                coords.as_slice()
            );
    ...
    #[test]
    fn test_quantized_coords_clamping() {
        // Test values outside [-1, 1] get clamped
        let values = vec![2.0; 128]; // Beyond valid range
        let embedding = Embedding::new(values).unwrap();

        let coords = QuantizedCoords::from_embedding(&embedding, 5, 10).unwrap();

        let expected_max = (1u16 << 10) - 1;
        assert!(
            coords.as_slice().iter().all(|&c| c == expected_max),
            "Values > 1.0 should clamp to max"
        );
    }
    ...
    #[test]
    fn test_quantized_coords_dims_exceed_embedding() {
        let values = vec![0.0; 128];
        let embedding = Embedding::new(values).unwrap();

        let result = QuantizedCoords::from_embedding(&embedding, 200, 10);
        assert!(
            matches!(result, Err(ElidError::InvalidDimension { .. })),
            "Should reject dims > embedding dimension"
        );
    }
    ...
    #[test]
    fn test_quantized_coords_1_bit() {
        // 1 bit: only 0 or 1
        let values = vec![-0.5, 0.0, 0.5, 1.0, -1.0];
        let values = values.into_iter().cycle().take(128).collect::<Vec<_>>();
        let embedding = Embedding::new(values).unwrap();

        let coords = QuantizedCoords::from_embedding(&embedding, 5, 1).unwrap();

        // 1 bit: max_val = 1
        // -0.5 → 0.25 * 1 = 0.25 ≈ 0
        // 0.0 → 0.5 * 1 = 0.5 ≈ 0 or 1 (boundary)
        // 0.5 → 0.75 * 1 = 0.75 ≈ 1
        // 1.0 → 1.0 * 1 = 1.0 = 1
        // -1.0 → 0.0 * 1 = 0.0 = 0

        assert_eq!(coords.as_slice()[0], 0, "Expected 0 for -0.5");
        assert!(coords.as_slice()[1] <= 1, "Expected 0 or 1 for 0.0");
        assert_eq!(coords.as_slice()[2], 1, "Expected 1 for 0.5");
        assert_eq!(coords.as_slice()[3], 1, "Expected 1 for 1.0");
        assert_eq!(coords.as_slice()[4], 0, "Expected 0 for -1.0");
    }

    #[test]
    fn test_quantized_coords_16_bits() {
        // Test maximum bit width
        let values = vec![0.0, 0.5, 1.0, -1.0];
        let values = values.into_iter().cycle().take(128).collect::<Vec<_>>();
        let embedding = Embedding::new(values).unwrap();

        let coords = QuantizedCoords::from_embedding(&embedding, 4, 16).unwrap();

        let max_val = (1u32 << 16) - 1; // 65535

        // 0.0 → midpoint ≈ 32768
        assert!(
            (coords.as_slice()[0] as i32 - 32768).abs() <= 1,
            "Expected ~32768 for 0.0"
        );

        // 0.5 → 0.75 * 65535 ≈ 49151
        assert!(
            (coords.as_slice()[1] as i32 - 49151).abs() <= 1,
            "Expected ~49151 for 0.5"
        );

        // 1.0 → max
        assert_eq!(coords.as_slice()[2] as u32, max_val, "Expected max for 1.0");

        // -1.0 → 0
        assert_eq!(coords.as_slice()[3], 0, "Expected 0 for -1.0");
    }

    #[test]
    fn test_quantized_coords_as_slice_zero_copy() {
        let values = vec![0.0; 128];
        let embedding = Embedding::new(values).unwrap();

        let coords = QuantizedCoords::from_embedding(&embedding, 10, 10).unwrap();

        // Verify we get a slice reference (zero-copy)
        let slice1 = coords.as_slice();
        let slice2 = coords.as_slice();

        assert_eq!(slice1.len(), 10);
        assert_eq!(slice2.len(), 10);
        assert_eq!(slice1.as_ptr(), slice2.as_ptr(), "Should be same memory");
    }

    #[test]
    fn test_quantized_coords_realistic_scenario() {
        // Simulate realistic embedding values from a normalized model
        let values: Vec<f32> = (0..768).map(|i| (i as f32 / 768.0) * 2.0 - 1.0).collect();

        let embedding = Embedding::new(values).unwrap();

        // Common Morton/Hilbert configuration
        let coords = QuantizedCoords::from_embedding(&embedding, 10, 10).unwrap();

        assert_eq!(coords.len(), 10);
        assert_eq!(coords.bits_per_dim(), 10);

        // Verify quantization is monotonic for the first 10 values
        let slice = coords.as_slice();
        for i in 1..slice.len() {
            assert!(
                slice[i] >= slice[i - 1],
                "Quantization should be monotonic for sorted input"
            );
        }
    }

    #[test]
    fn test_quantized_coords_precision() {
        // Test that quantization precision is reasonable
        let values = vec![0.1; 128];
        let embedding = Embedding::new(values).unwrap();

        let coords = QuantizedCoords::from_embedding(&embedding, 5, 10).unwrap();

        // 0.1 → (0.1 + 1) / 2 * 1023 = 0.55 * 1023 = 562.65 ≈ 563
        let expected = 563u16;
        assert!(
            coords
                .as_slice()
                .iter()
                .all(|&c| (c as i32 - expected as i32).abs() <= 1),
            "Expected ~{}, got {:?}",
            expected,
            coords.as_slice()
        );
